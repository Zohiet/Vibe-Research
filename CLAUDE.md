...
⚠️ **应当全绿，没有已知失败、没有豁免白名单**（用例数以 `./ci.ps1` 实际输出为准——
这里曾写死过一个数字，一天之内就从 86 漂到 177，成了第二处会过时的真相源）。 任何一条挂了都要修——不要往 `ci.ps1` 或文档里加「这条不用管」的例外。（本仓库曾养过一条 Windows 专属失败，代价是同一句解释散在五处文档里；根因修掉后不再引入这类豁免。）
...
- **紧迫色 `--due-1/2/3`（VR-GOAL-024）**：自选股「下次财报」列按剩余天数分三档，
  5 天内才上色。三条约束：**颜色是冗余强化**（格子里本来就写着天数，色觉障碍用户
  读数字不受影响）；**只表达「临近」这个时间事实**，已过预约日的**不上色**
  （那会带上"这家不对劲"的褒贬）；用 `primary` 色系而非 `warning`（后者自带警惕意味）。
  ⚠️ **两个主题的色阶方向相反**——亮色越紧迫越深、暗色越紧迫越亮，
  一套值套两个主题会让其中一个反掉且**不会有任何报错**。
  目标对比度取**等比数列**（5.0 / 8.0 / 12.8）而非等差：两个 token 的互相对比度
  恒等于它们各自对卡片对比度的**比值**，等差换算下来只有 1.35，达不到「相邻档 ≥1.5」。
...
- **自选股表有宽度护栏**（VR-GOAL-027）：`VR-GOAL-027_forward-pe.spec.ts` 断言
  表格内容宽度不超过容器可用宽度。**它红了不代表实现坏了，代表这张表满了**——
  那时正确的动作是先回答「砍哪列」，而不是顺手再加宽一次容器。
  该表已被连续五个 Goal 加到 22 列（实测 1574px / 可用 1616px，余量很小）。
...
**这几条命令是一条流水线，不是彼此独立的工具。** 一个 Goal 从头走到尾会依次经过它们：

```
阶段 1-3  开 Goal / 写 Spec / 写 Plan / 实现   →  /vr-goal   （写完 Plan 停下等第二道闸）
阶段 4    验收：CI + 证据 + 写验收报告          →  /vr-accept ← 实现完就走这条，别手写
阶段 5    发布 dev → main                      →  /vr-release（用户主动走）
```

⚠️ **实现完了不要自己手写验收。** VR-GOAL-008~011 连续四轮跳过 `/vr-accept` 手写验收，
靠"照抄上一个 Goal"决定报告写哪儿，抄到的样板恰好是另一个档位的——三份报告全写错位置，
一直到发布前才被抓到。**流程的知识在命令里，不在你对上一个 Goal 的印象里。**

各阶段**具体要求什么产物、报告写哪儿、档位怎么分**，唯一真相源是
[`goal_workflow.md`](docs/harness/goal_workflow.md) 的「完成定义」一节——
**本文件只给地图，不复述规则**（规则有两份就一定会漂移，这已经发生过）。
...
- **打板原始池**（`astock.em_zt_topic_pool`）含个股 code/name。规则有两层，别混着记：
  - **原始池不得绕过 `market.py`**——只有 `astock.py`（定义）与 `market.py`（消费）能碰它，
    出现在 `app.py` / `tools.py` / `mcp_server.py` 即违规。有 `test_zt_pool_containment.py` 静态拦截，
    新增消费方要显式改它的 `ALLOWED`（那一步是有意的确认动作）。
  - **`market.py` 输出什么由产品定**：既有不含个股名的情绪指标（封板率 / 炸板率 / 连板梯队），
    **也有含个股名的客观公开榜单**（连板股清单、成交额 TOP20）——2026-07-05 的产品定位调整，
    从「零标的」改为「展示客观榜单但不推荐 / 不预测 / 不评分」。
  > ⚠️ 这里此前写的是「仅供聚合成**不含个股名**的情绪指标」，而 `lianban_stocks` 早就在
  > 输出个股名、前端也在渲染——**文档只吸收了那次产品调整的一半**，VR-GOAL-025 校正。
