...
> **真实案例：连续四轮把验收报告写错地方（VR-GOAL-008~011）。**
> 三个**完整档**的验收报告全被写进了 Goal Spec 末尾，而那是**轻量档**的落点。
> 原因不是不知道规则——`/vr-accept` 白纸黑字写着位置——而是**这四个 Goal 一次
> `/vr-accept` 都没走过**，验收全靠"看上一个 Goal 长什么样照抄"，
> 而抄到的样板（VR-GOAL-008）恰好是轻量档、它自己完全正确。
> **错误从一个正确的样板里长出来，看起来毫无破绽。**
>
> 三层教训：
> 1. **样板不能代替规则。** 上一个 Goal 可能属于另一个档位，它对不代表你对。
> 2. **规则不能有两份。** 当时 `/vr-accept` 与 `/vr-release` 都复述了完成定义，
>    且都漏掉了档位分岔——**规范是对的，两条命令都错**。现在两条命令都不复述，
>    只负责把你送到这一节来读原文。
> 3. **自证的核对等于没有核对。** 那三份报告里连着三次写「完成定义已满足」，
>    三次都是假的。真正拦住它的是 `/vr-release` 的**独立**前置复查——
>    整条流水线上唯一一道不是"自己查自己"的关卡。**别把那一步做成走过场。**
>
> 修复见 VR-GOAL-012。
