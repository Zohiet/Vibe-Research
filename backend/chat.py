...
# 机构目标价的口径 —— **三条 AI 出口共用这一份**（chat / debate / reflection）。
#
# ⚠️ 提成常量不是为了少打字。VR-GOAL-023 补口径时只改了 `SYSTEM_PROMPT` 这一处，
# 而 `debate._COMMON_RULES` 里那句一刀切的「不给目标价」**悄悄和它分了家**，
# 没有任何东西报警——直到 VR-GOAL-025 才发现。同一句话写三份，就是把
# 「它会漂」这个赌注下三次；写一份、三处引用，漂移在结构上就不可能发生。
#
# 放在 `chat.py` 而不是新开模块：`debate.py` 与 `reflection.py` 本就 `import chat`
# （两者都调 `chat._call_llm_stream`），零新增模块、零新增依赖边。
TARGET_PRICE_RULE = (
    "机构目标价：**可以转述**工具或底稿里查到的机构目标价，但必须标明是哪家机构、"
    "什么日期给的、共有几家给价；超过 90 天的要说明它是旧观点。"
    "**不自行推算目标价，也不计算目标价相对现价的涨跌空间**——那个数没有任何机构说过。"
)
...
输出组织（像专业研报那样排版，但只陈述客观事实、不给买卖建议、不自行给出评级或目标价）：
...
- {TARGET_PRICE_RULE}用户看到的页面上就是这个口径，你的回答要和它一致。
