...
- 不预测股价涨跌与具体价位、不给买卖时机、不给仓位建议、不承诺收益。
- {chat.TARGET_PRICE_RULE}
- 用简洁中文，条目化输出。
"""
# ⚠️ 上面那条目标价规则**必须引用 `chat.TARGET_PRICE_RULE`，不许在这里另写一份**。
# 此处原本是一刀切的「不给目标价」，而底稿里含 `query_reports`（见 `_FACT_SPECS`），
# VR-GOAL-023 给研报补了 `indvAimPriceT` 之后，多空双方**拿到了目标价却被告知不许提**
# ——数据给了又不让用，正是 023 决策 13 判为自相矛盾的同一形态。
# `test_ai_outlet_discipline.py` 里有护栏钉着这个引用关系。
